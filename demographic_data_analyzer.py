...
def calculate_demographic_data(print_data=True):
    ...
    race_count = df.unique(pd.Series('race')).count()
    ...
    tmp_df = df[df['salary'] == '>50K']

    highest_earning_country = tmp_df.value_counts('native-country').index[0]
    highest_earning_country_percentage = round(tmp_df.value_counts('native-country', normalize=True).values[0] * 100, 1)
